...
// ── Adherence ring ─────────────────────────────────────────────────────────────
function AdherenceRing({ percent = 0 }: { percent?: number }) {
  const r = 18;
    ...
    <div style={{ padding: "12px 14px", background: "rgba(99,102,241,.07)", borderRadius: 12, border: "1px solid rgba(99,102,241,.14)" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 11, color: T.muted, fontFamily: "var(--font-ui,'Inter',sans-serif)" }}>this week</div>
        </div>
      </div>
    </div>
  );
                ...
                {NAV_ITEMS.map((it) => {
                  ...
                  return (
                    <Link key={it.href} href={it.href} onClick={() => setDrawerOpen(false)}
                      ...
                      <span style={{ fontSize: 15 }}>{it.icon}</span>
                      <span style={{ flex: 1 }}>{it.label}</span>
                      {it.badge && <span style={{ fontSize: 10, padding: "2px 6px", borderRadius: 100, background: "rgba(239,68,68,.18)", color: "#F87171", fontWeight: 400 }}>{it.badge}</span>}
                    </Link>
                  );
                })}
              </nav>
