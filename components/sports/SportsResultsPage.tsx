...
const PHASE_COLORS: Record<string, { bg: string; border: string; label: string }> = {
  "Base":                  { bg: "rgba(59,130,246,.12)",  border: "rgba(59,130,246,.4)",  label: "#60A5FA" },
  "Build":                 { bg: "rgba(99,102,241,.12)",  border: "rgba(99,102,241,.4)",  label: "#818CF8" },
  "Peak":                  { bg: "rgba(124,58,237,.14)",  border: "rgba(124,58,237,.5)",  label: "#A78BFA" },
  "Taper":                 { bg: "rgba(168,85,247,.12)",  border: "rgba(168,85,247,.4)",  label: "#C084FC" },
  "Race Week":             { bg: "rgba(239,68,68,.12)",   border: "rgba(239,68,68,.5)",   label: "#F87171" },
  "Post-Event Recovery":   { bg: "rgba(16,185,129,.1)",   border: "rgba(16,185,129,.35)", label: "#34D399" },
};
...
// ── Section 1: Periodized Timeline ────────────────────────────────────────────
function TimelineSection({ phases }: { phases: SportsTimelinePhase[] }) {
...
function CompetitionPackSection({ tierPack }: { tierPack: SportsProtocolPayload["tierPack"] }) {
        ...
        {tab === "testing" && (
          <ul style={{ margin: 0, padding: "0 0 0 16px" }}>
            {tierPack.testing.map((t, i) => (
              <li key={i} style={{ fontSize: 13, color: T.text, fontFamily: "var(--font-ui,'Inter',sans-serif)", fontWeight: 300, lineHeight: 1.7 }}>{t}</li>
            ))}
          </ul>
        )}
        ...
        {tab === "gear" && (
          <ul style={{ margin: 0, padding: "0 0 0 16px" }}>
            {tierPack.gear.map((g, i) => (
              <li key={i} style={{ fontSize: 13, color: T.text, fontFamily: "var(--font-ui,'Inter',sans-serif)", fontWeight: 300, lineHeight: 1.7 }}>{g}</li>
            ))}
          </ul>
        )}
...
function SupplementScheduleSection({ schedule }: { schedule: SportsSupplementItem[] }) {
            ...
            <button key={i} onClick={() => setExpanded(isOpen ? null : i)}
              ...
              {isOpen ? (
                ...
                <div style={{ fontSize: 12, color: T.muted, fontFamily: "var(--font-ui,'Inter',sans-serif)", fontWeight: 300, lineHeight: 1.4 }}>
                  {m.trainingGuidance.slice(0, 70)}{m.trainingGuidance.length > 70 ? "…" : ""}
                </div>
              )}
            </button>
          );
...
// ── Competition type display labels ───────────────────────────────────────────
...
function EventBanner({ meta }: { meta: EventMeta }) {
  const date = (() => {
  ...
  const label = COMPETITION_LABELS[meta.competitionType] ?? meta.competitionType.replace(/_/g, " ").replace(/\b\w/g, (c) => c.toUpperCase());
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
        <div>
          ...
          <div style={{ fontSize: 13, color: T.muted, fontFamily: "var(--font-ui,'Inter',sans-serif)", fontWeight: 300 }}>
            {date} · {meta.weeksToEvent} weeks away · {meta.priorityOutcome}
          </div>
        </div>
...
export function SportsResultsPage({
  ...
  return (
    <div className="px-4 lg:px-6 py-6 lg:py-8">
      <EventBanner meta={eventMeta} />

      <TimelineSection phases={payload.periodizedTimeline} />
      <RedFlagsSection redFlags={payload.redFlags} />
      <CompetitionPackSection tierPack={payload.tierPack} />
